...
public class Cfg2VcmlTImportWizard extends Wizard implements IImportWizard {
	...
	@Inject
	private Cfg2VcmlTImport cfg2vcmltImport;
		...
		try {
			getContainer().run(false, false, new IRunnableWithProgress() {
				public void run(final IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					monitor.beginTask("Transforming cfg/xml files to VCMLT files...", IProgressMonitor.UNKNOWN);
					...
					for(File file : page.getFilesToTransform()) {
						try {
							cfg2vcmltImport.doImport(referencedModel, file, vcmltModel);

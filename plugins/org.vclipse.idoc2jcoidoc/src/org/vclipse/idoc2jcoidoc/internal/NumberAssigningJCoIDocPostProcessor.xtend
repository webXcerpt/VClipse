...
/** 
 */
class NumberAssigningJCoIDocPostProcessor implements IJCoIDocPostprocessor {
	/** 
	 */
	final String upsNumber
	/** 
	 */
	BigInteger currentIDocNumber

	/** 
	 * @param upsNumber
	 * @param initialIDocNumber
	 */
	new(String upsNumber, BigInteger initialIDocNumber) {
	 ...
	 */
	override void postprocess(List<IDocDocument> iDocDocuments) throws IDocException {
		val Map<BigInteger, BigInteger> iDocNumbers = new HashMap<BigInteger, BigInteger>()
		for (IDocDocument iDocDocument : iDocDocuments) {
			currentIDocNumber = currentIDocNumber.add(BigInteger::valueOf(1))
			iDocNumbers.put(new BigInteger(iDocDocument.getIDocNumber()), currentIDocNumber) // set IDoc numbers
			// in documents
			iDocDocument.setIDocNumber(currentIDocNumber.toString())
			if (upsNumber !== null) {
				// set UPS number
				val IDocSegment segmentRoot = iDocDocument.getRootSegment()
				...
				if ("UPSMAS01".equals(iDocDocument.getIDocType())) {
					for (IDocSegment segmentE1UPSHDR : iDocDocument.getRootSegment().getChildren("E1UPSHDR")) {
						for (IDocSegment segmentE1UPSITM : segmentE1UPSHDR.getChildren("E1UPSITM")) {
							val BigInteger oldIDocNumber = segmentE1UPSITM.getBigInteger("SNDDOC")
							val BigInteger newIDocNumber = iDocNumbers.get(oldIDocNumber)
							if (newIDocNumber === null) {
								throw new RuntimeException(
									'''IDoc number not found: «oldIDocNumber» «iDocNumbers»'''.toString) // TODO have some exception
							} else {

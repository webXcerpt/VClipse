...
/** 
 */
class ConnectionsExportWizardPage extends AbstractWizardPage {
	/** 
	 */
	boolean exportPasswords
	/** 
	 */
	 ...
	 * @param pageName
	 */
	@Inject protected new(String pageName, IConnectionHandler connectionHandler) {
		super(pageName)
	...
	override void createControl(Composite parent) {
		...
		val GridData gridData = new GridData(GridData::FILL_HORIZONTAL)
		gridData.horizontalSpan = 2
		var Button button = new Button(connectionsArea, SWT::CHECK)
		button.setText("Overwrite existing file")
		button.setLayoutData(gridData)
		button.addSelectionListener(new SelectionAdapter() {
			override void widgetSelected(SelectionEvent event) {
				overwriteExistingFile = (event.widget as Button).getSelection()
			}
		})
		...
		button.addSelectionListener(new SelectionAdapter() {
			override void widgetSelected(SelectionEvent event) {
				exportPasswords = (event.widget as Button).getSelection()
			}
		})
	 ...
	 * @return
	 */
	def AbstractConnection[] getConnections2Export() {
	 ...
	 * @return
	 */
	def boolean overwriteExistingFile() {
